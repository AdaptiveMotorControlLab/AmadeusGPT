...
class IntegrationModuleHub:
    ...
    def save_embeddings(self):
        ...
        if len(result) > 0:
            with open(os.path.join(self.amadeus_root, "modules_embedding.pickle"), "wb") as file:
    ...
    def match_module(self, query):
        ...
        query_embedding = client.embeddings.create(input=[query], model=model).data[
                0
        ].embedding

        if not os.path.exists(os.path.join(self.amadeus_root, "modules_embedding.pickle")):
            self.save_embeddings()

        with open(os.path.join(self.amadeus_root, "modules_embedding.pickle"), "rb") as f:
