...
We have two sets of keypoint space, one for MABe and the other for SuperAnimalTopViewMouse
"""
...
def get_chase_events(config):
    ...
    chase_events = analysis.event_manager.get_animals_animals_events(
        ["closest_distance<40", "orientation==Orientation.FRONT"],
        smooth_window_size=25,
        min_window=30,
    )
    speeding_events = analysis.event_manager.get_animals_state_events("speed>=3.4")

    chase_events = analysis.event_manager.get_composite_events(
        chase_events, speeding_events, composition_type="logical_and"
    )
...
def get_oral_genital_contact_events(config):
    ...
    analysis = AnimalBehaviorAnalysis(config)
    get_oral_genital_contact_events = analysis.event_manager.get_animals_animals_events(
        ["closest_distance<15"],
        bodypart_names=["nose"],
...
def get_oral_oral_contact_events(config):
    ...
    """
    analysis = AnimalBehaviorAnalysis(config)
    get_oral_oral_contact_events = analysis.event_manager.get_animals_animals_events(
        ["closest_distance<15"],
        bodypart_names=["nose"],
...
def get_approaches_events(config):
    ...
    """
    analysis = AnimalBehaviorAnalysis(config)
    distance_events = analysis.event_manager.get_animals_animals_events(["distance>40"])

    close_distance_events = analysis.event_manager.get_animals_animals_events(
        ["distance<8"]
    )

    approaches_events = analysis.event_manager.get_composite_events(
        distance_events, close_distance_events, composition_type="sequential"
    )
...
def get_huddles_events(config):
    ...
    """
    analysis = AnimalBehaviorAnalysis(config)
    huddles_events = analysis.event_manager.get_animals_animals_events(
        ["distance<50", "relative_speed<4"], min_window=75, smooth_window_size=61
    )
...
def get_contact_events(config):
    ...
    """
    analysis = AnimalBehaviorAnalysis(config)
    contact_events = analysis.event_manager.get_animals_animals_events(
        ["closest_distance<12"], smooth_window_size=11, min_window=5
    )
...
def get_watching_events(config):
    ...
    """
    analysis = AnimalBehaviorAnalysis(config)
    watching_events = analysis.event_manager.get_animals_animals_events(
        ["distance>260", "distance>50", "relative_head_angle<15"],
        min_window=100,
...
def get_oral_ear_contact_events(config):
    ...
    analysis = AnimalBehaviorAnalysis(config)
    get_oral_ear_contact_events = analysis.event_manager.get_animals_animals_events(
        ["closest_distance<10"],
        bodypart_names=["nose"],
