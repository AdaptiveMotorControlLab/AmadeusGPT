...
from amadeusgpt.utils import (get_fps,
    filter_kwargs_for_function, get_video_length
)
...
class SceneVisualization(MatplotlibVisualization):
    ...
    def __init__(self,
            ...
            self.scene_frame = self._get_scene_frame(video_file_path,
                                                    scene_frame_number)
...
class KeypointVisualization(MatplotlibVisualization):
    ...
    def _plot_trajectory(
        ...
        **kwargs,
        ...
        if self.average_keypoints: 
            ...
            bodypart_indices = [self.full_bodypart_names.index(bodypart) for bodypart in self.bodypart_names_to_draw]
    ...
    def _event_plot_trajectory(self,
                                **kwargs):

        data = self.keypoints
        events = self.events
        events = [event for event in events if event.sender_animal_name == self.sender_animal_name and \
                    self.receiver_animal_name.issubset(event.receiver_animal_names)]

        self.axs.set_title('' if len(self.receiver_animal_name) == 0 else list(self.receiver_animal_name)[0])
...
def _plot_ethogram(etho_obj: Dict[str, Any],
                   ax,
                  video_path, cmap="rainbow"):
...
class GraphVisualization(MatplotlibVisualization):
    ...
    def update(self, time):
        ...
        nodes_to_display = [n for n, d in self.nx_graph.nodes(data=True) if d['start'] <= time]
        ...
        for time in sorted_times:
            ...
            if time != last_time:
                ...
                last_time = time
                current_x_position = x_index * x_spacing  # Calculate new x position based on time
        ...
        nx.draw(subG, pos, ax=self.axs, with_labels=True, node_color='lightblue', edge_color='gray', font_size=10)
